...
class WorkStore:
    ...
    def save(self, work: Work) -> None:
        ...
        self.conn.executemany("INSERT INTO provenance VALUES (?, ?, ?, ?, ?)", [(work.id, i, p.get("capability_id"), p.get("capability_name"), json.dumps(p)) for i, p in enumerate(work.provenance)])
        self.conn.commit()
    ...
    def load(self, work_id: str) -> Work:
        ...
        work.provenance = [json.loads(r[4]) for r in self.conn.execute("SELECT seq, work_id, capability_id, capability_name, data FROM provenance WHERE work_id=? ORDER BY seq", (work_id,))]
        return work
